...
<div class="min-h-screen bg-background p-8">
  <div class="max-w-6xl mx-auto">
    <h1 class="text-4xl font-bold text-theme-primary mb-2">Bobbleheads Checklist</h1>
    ...
    <!-- SPECIAL Bobbleheads -->
    <div class="mb-8">
      <h2 class="text-2xl font-bold text-theme-primary mb-4">SPECIAL Bobbleheads</h2>
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        {#each bobbleheads.filter(b => b.category === 'SPECIAL') as bobblehead (bobblehead.id)}
          <div class="bg-surface rounded-lg p-4 hover:bg-border transition-colors">
            <label class="flex items-start gap-3 cursor-pointer">
              <input
                type="checkbox"
                checked={$checkedBobbleheads.has(bobblehead.id)}
                on:change={() => toggleBobblehead(bobblehead.id)}
                class="mt-1 w-5 h-5 rounded accent-theme-primary"
              />
              <div class="flex-1">
                ...
                <div class="text-sm text-theme-primary mt-2">{bobblehead.effect}</div>
              </div>
            </label>
          </div>
        {/each}
    ...
    <!-- Skill Bobbleheads -->
    <div>
      <h2 class="text-2xl font-bold text-theme-primary mb-4">Skill Bobbleheads</h2>
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
